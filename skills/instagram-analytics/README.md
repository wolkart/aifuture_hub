...
**Папка на блогера:** `<OUTPUT_DIR>/<author>/index.md` (ранжированная таблица, колонка «Тема» = кликабельная ссылка в карточку) + `cards/<id>.md` (карточка: `aliases`+читаемый заголовок, при `enrich` — транскрипт + **дословный перевод на русский** + разбор) + `export.csv` (для Sheets). Плюс `<OUTPUT_DIR>/_Разведка.base` — Obsidian Base со сводным кросс-блогерным срезом (фильтр по автору/ER). Видео не хранятся (только `video_url`). Единая схема — мостик к будущей БД/веб-интерфейсу.
