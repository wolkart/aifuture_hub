...
## Как читать `.env` (не через `source`)

`OUTPUT_DIR` — путь с пробелами и без кавычек, поэтому `source .env` падает (`command not found: Automation`). Тяни нужные ключи по одному:

```bash
ENVF="<путь к скиллу>/.env"
APIFY_TOKEN=$(grep -E '^APIFY_TOKEN=' "$ENVF" | cut -d= -f2-)
APIFY_ACTOR=$(grep -E '^APIFY_ACTOR=' "$ENVF" | cut -d= -f2-)
```
